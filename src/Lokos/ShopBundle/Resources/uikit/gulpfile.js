...
gulp.task('dist', ['dist-themes-core'], function(done) {
    ...
    runSequence('dist-core-minify', function(){
...
gulp.task('dist-core-minify', function(done) {
    ...
    // minify css
    gulp.src(['!./dist/css/**/*.min.css', './dist/css/**/*.css']).pipe(rename({ suffix: '.min' })).pipe(minifycss({advanced:false})).pipe(gulp.dest('../public/css')).on('end', function(){

        // minify js
        gulp.src(['!./dist/js/**/*.min.js', './dist/js/**/*.js']).pipe(rename({ suffix: '.min' })).pipe(uglify()).pipe(gulp.dest('../public/js')).on('end', function(){
            return gulp.src(['./dist/fonts/**']).pipe(gulp.dest('../public/fonts'));
        });
    });
});
